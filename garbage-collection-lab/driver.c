...
void speed(trace_t *ptrace, stats_t *sp) {
    trace = ptrace;
    sp->execution_time = ftime_default(speed_funct, ftime_tolerance);
...
//        switch (c) {
//            case 'd': dump = 1; break;
//            case 'p': pgms_only = 1; break;
//            case 's': traces_only = 1; break;
//            case 't': ftime_tolerance = atof(optarg); break;
...
//    if (verbose) printf("Running in verbose mode (use -q for quiet mode)...\n\n");
//    mem_init(); /* Initialize memlib */
//
//    /* initilize stats */
